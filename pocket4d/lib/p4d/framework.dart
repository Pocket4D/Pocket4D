...
function loadPage(pageId) {
    ...
    class P4D {
        constructor(pageId) {
            this.onNetworkResult = (requestId, result, json) => {
    ...
    class RealPage {
        constructor(pageId) {
            this.__native__evalInPage = (jsContent) => {
                if (!jsContent) {
                    console.log('js content is empty!');
                }
                eval(jsContent);
            };
            this.__native__getExpValue = (id, type, prefix, watch, script) => {
                if (watch === true) {
            ...
            this.__native__removeObserverByIds = (ids) => {
                this.observer.removeWatcher(ids);
            };
            this.setTimeout = (callback, ms, ...args) => {
                let timerId = guid();
            ...
            this.setInterval = (callback, ms, ...args) => {
                let timerId = guid();
                callbacks[timerId] = callback;
                callbackArgs[timerId] = args;
